...
//glClear( GL_DEPTH_BUFFER_BIT|GL_STENCIL_BUFFER_BIT );
   ...
   // Drawarrow3D({0,0,0},{100,0,0},rojo,0.03,0.003);
   // Drawarrow3D({0,0,0},{0,100,0},verde,0.03,0.003);
  //  Drawarrow3D({0,0,0},{0,0,100},azul,0.00,0.003);
/// //////////////////////////// PARTES A AGREGAR
   ///SSRMS.Parametrica();
   //SSRMS.InverseKinematics();
SSRMS.renderizar();
SSRMS.DibujarCurva();
   //SSRMS.t=SSRMS.t-0.08;

}
void ClaseOpenGL::preparar(float dt){
//SSRMS.preparar();  //runge kutta
SSRMS.Move();      //inverse kinematics
}
void ClaseOpenGL::Resize(int width, int height){
  ...
  glViewport(0, 0, width, height);		// reset the viewport to new dimensions
        ...
        gluPerspective(52.0f,(GLdouble)width/(GLdouble)height,.001f,10000.0f);
